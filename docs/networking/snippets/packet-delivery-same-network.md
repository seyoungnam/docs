...
![Local Network](../../assets/img/networking/snippets/local-network.svg)
...
* **Addressing Labels:**
    ...
    * **Destination IP:** `192.168.8.107` (Computer B)


??? info "Packet Flow Decisions based on Destination IP"

    ![eno1 for dst IP inside of the network](../../assets/img/networking/snippets/choose-eno1-interface.svg)
...
* **Addressing Labels:**
    ...
    * **Destination MAC:** `fe:d0:b0:80:c3:d8` (Computer B's NIC)

``` bash title="ARP table example(IP -> MAC)" hl_lines="2 4"
$ arp -a
...
![Transmitting within local network](../../assets/img/networking/snippets/transmitting.svg)
...
* **The Switch's Job:** It acts like a smart traffic cop. It does **not** read the IP addresses or the inner data. {++It only reads the first few bytes to find the **Destination MAC Address** (`fe:d0:b0:80:c3:d8`)++}.
...
* **The Security Check:** It reads the **Destination MAC**. Because it matches `fe:d0:b0:80:c3:d8`, it accepts the frame, strips the **Ethernet Header** away, and passes the payload up.

### Step 7: Layer 3 Verification (Frame $\rightarrow$ Packet)

* **Action:** The operating system processes the inner **IP Packet**.
* **The Security Check:** It reads the **Destination IP** (`192.168.8.107`). It matches its own IP, so it accepts the packet, strips the **IP Header** away, and passes the segment up.
