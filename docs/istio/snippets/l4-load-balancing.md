...
![L4 Connection Lifecycle](../../assets/img/istio/snippets/l4-conn-flow.svg)

The diagram below provides a detailed view of the L4 connection lifecycle within the client-side Envoy proxy.

![L4 Connection Lifecycle within Envoy](../../assets/img/istio/snippets/l4-conn-flow-details.svg)
...
For industrial IoT applications handling telemetry ingestion or real-time control loops, standard Round-Robin load balancing is often sub-optimal. Envoy provides highly optimized algorithms in C++:
...
*   **Why it's critical:** In a **Stateful Telemetry Ingestion** pipeline, you want a specific edge device or smart sensor to always connect to the *same* backend ingestion worker instance. This allows the backend to keep a warm memory cache of that specific device's real-time configuration or local state.
...
In IoT and distributed systems, **gRPC (HTTP/2)** is heavily used for streaming telemetry and remote procedure calls because of its efficiency. However, a major architectural trap occurs when attempting to balance gRPC traffic at Layer 4.
...
!!! Danger "The L4 gRPC Imbalance Trap"
    ...
    If 10 client edge devices connect via gRPC through an L4 balancer to a backend pool of 10 pods, all 10 connections might land on just 2 or 3 pods. As a result, those 3 pods will spike to 100% CPU, while the remaining 7 pods sit completely idle.
...
When configuring Istio's `DestinationRule` under extreme TCP connection scales (e.g., millions of concurrent edge device sessions), you must tune the low-level connection pool limits in the Istio Go Control Plane (`istiod`), which translates them to Envoy's `v3.Cluster.Thresholds` configuration:
